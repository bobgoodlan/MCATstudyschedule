...
for idx, row in df_shifted.iterrows():
    ...
    if t in shift_conference_types and in_any_range(d, conf_ranges):
        while in_any_range(d, conf_ranges): d += timedelta(days=1)
...
for vr_start, vr_end in vac_ranges_sorted:
    ...
    for _, r in df_shifted.iterrows():
        if r['Date'] > vr_end: slot_counts[r['Date']] += 1
    for i in sorted(vac_idxs, key=lambda x: df_shifted.at[x, 'Date']):
        cand = vr_end + timedelta(days=1)
        while slot_counts[cand]>=6 or in_any_range(cand, conf_ranges): cand += timedelta(days=1)
        df_shifted.at[i, 'Date'] = cand
        slot_counts[cand]+=1
# Avoid Study Date collisions
if 'Study Date' in shift_conference_types:
    ...
    for idx, r in df_shifted[df_shifted['Task Type']=='Study Date'].sort_values('Date').iterrows():
        ...
        if d in occupied or in_any_range(d, conf_ranges):
            while d in occupied or in_any_range(d, conf_ranges): d+=timedelta(days=1)
            df_shifted.at[idx,'Date']=d
        occupied.add(d)
...
# ───────── Color Map ─────────
color_map={
    'Study Date':'#1f77b4','1-Day Review':'#ff7f0e','3-Day Review':'#2ca02c',
    '7-Day Review':'#d62728','14-Day Review':'#9467bd','30-Day Review':'#8c564b',
    '60-Day Review':'#e377c2','Final Review':'#7f7f7f','Full Length Exam':'#4361ee'
}
...
if total_remaining==0: st.info("🎉 All tasks for this week are completed!")
cols=st.columns(7)
    ...
    with cols[i]:
        ...
        if not d_tasks:
            ...
            for idx,(t,top) in enumerate(d_tasks):
                ...
                if not comp:
                    st.markdown(f"<div style='display:inline-block;background-color:{color};color:white;padding:4px 8px;border-radius:4px;font-size:0.9em;font-weight:500;margin-left:6px;margin-bottom:4px;'>{t}</div>",unsafe_allow_html=True)
                    if top: st.markdown(f"<div style='margin-left:12px;font-size:0.9em;color:#e0e0e0;margin-bottom:12px;'>{top}</div>",unsafe_allow_html=True)
                else:
                    st.markdown(f"<div style='display:inline-block;background-color:lightgray;color:#666;padding:4px 8px;border-radius:4px;font-size:0.9em;font-weight:500;margin-left:6px;margin-bottom:4px;text-decoration:line-through;'>{t}</div>",unsafe_allow_html=True)
                    if top: st.markdown(f"<div style='margin-left:12px;font-size:0.9em;color:#999;margin-bottom:12px;text-decoration:line-through;'>{top}</div>",unsafe_allow_html=True)
